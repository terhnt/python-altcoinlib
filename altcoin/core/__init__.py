...
import bitcoin.core
from bitcoin.core import b2lx, x, CTransaction
from bitcoin.core.serialize import ser_read, uint256VectorSerializer,\
    VectorSerializer
...
class CAuxPow(CTransaction):
    ...
    added directly here.
    """
    __slots__ = ['hashBlock', 'vMerkleBranch', 'nIndex', 'vChainMerkleBranch',
                 'nChainIndex', 'parentBlockHeader']

    def __init__(self, vin=(), vout=(), nLockTime=0, nVersion=1, hashBlock=0,
                 vMerkleBranch=None, nIndex=0, vChainMerkleBranch=(),
                 nChainIndex=0, parentBlockHeader=None):
    ...
    def __repr__(self):
        return "%s(%r, %r, %i, %i, lx(%s), %r, %i, %r, %i, %r)" % \
            (self.__class__.__name__, self.vin, self.vout, self.nLockTime,
             self.nVersion, b2lx(self.hashBlock),
             self.vMerkleBranch, self.nIndex, self.vChainMerkleBranch,
             self.nChainIndex, self.parentBlockHeader)
...
class CAltcoinBlockHeader(bitcoin.core.CBlockHeader):
    ...
    def __init__(self, nVersion=2, hashPrevBlock=b'\x00'*32,
                 ...
                 auxpow=None):
        super(CAltcoinBlockHeader, self).__init__(nVersion, hashPrevBlock,
    ...
    difficulty = property(lambda self:
                          CAltcoinBlockHeader.calc_difficulty(self.nBits))

    def __repr__(self):
        return "%s(%i, lx(%s), lx(%s), %s, 0x%08x, 0x%08x, %s)" % \
            (self.__class__.__name__, self.nVersion,
             b2lx(self.hashPrevBlock), b2lx(self.hashMerkleRoot),
...
class CoreUnoMainParams(bitcoin.core.CoreChainParams):
    NAME = 'unobtanium_main'
    GENESIS_BLOCK = CAltcoinBlock.deserialize(x(''))
    SUBSIDY_HALVING_INTERVAL = 100000
    PROOF_OF_WORK_LIMIT = 2**256-1 >> 20
    
    
class CoreUnoTestNetParams(CoreUnoMainParams):
    NAME = 'unobtanium_test'
    GENESIS_BLOCK = CAltcoinBlock.deserialize(x(''))
...
for params in [bitcoin.core.CoreMainParams(),
               bitcoin.core.CoreTestNetParams(),
               bitcoin.core.CoreRegTestParams(),
               CoreUnoMainParams(),
               CoreUnoTestNetParams(),
               CoreLtcMainParams(),
